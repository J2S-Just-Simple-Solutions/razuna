...
<!--- circuit: c --->
<!--- fuseaction: first_time_database_check --->
<cftry>
<cfset myFusebox.thisPhase = "appinit">
<cfset myFusebox.thisCircuit = "c">
<cfset myFusebox.thisFuseaction = "first_time_database_check">
...
<cfset theconnection = myFusebox.getApplicationData().global.verifydatasource() >
<!--- do action="ajax.first_time_database_check" --->
<cfset myFusebox.thisCircuit = "ajax">
<cftry>
<cfoutput><cfinclude template="../views/ajaxparts/dsp_firsttime_database_check.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 32 and right(cfcatch.MissingFileName,32) is "dsp_firsttime_database_check.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_firsttime_database_check.cfm in circuit ajax which does not exist (from fuseaction ajax.first_time_database_check).">
