...
<cfset myFusebox.thisFuseaction = "users">
...
<cfset xfa.rto = "c.users" />
<cfset attributes.rowmax = "30" />
<cfset attributes.rowmin = "0" />
<cfset qry_allhosts = myFusebox.getApplicationData().global.allhosts() >
<cfset qry_users = myFusebox.getApplicationData().users.getall(attributes) >
<!--- do action="ajax.users" --->
<cfset myFusebox.thisCircuit = "ajax">
<cftry>
<cfoutput><cfinclude template="../views/ajaxparts/dsp_users_main.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 18 and right(cfcatch.MissingFileName,18) is "dsp_users_main.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_users_main.cfm in circuit ajax which does not exist (from fuseaction ajax.users).">
