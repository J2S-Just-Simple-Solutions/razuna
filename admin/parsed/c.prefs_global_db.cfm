...
<!--- circuit: c --->
<!--- fuseaction: prefs_global_db --->
<cftry>
<cfset myFusebox.thisPhase = "appinit">
<cfset myFusebox.thisCircuit = "c">
<cfset myFusebox.thisFuseaction = "prefs_global_db">
...
<cfset gprefs = myFusebox.getApplicationData().Settings.get_global() >
<!--- do action="ajax.prefs_global_db" --->
<cfset myFusebox.thisCircuit = "ajax">
<cftry>
<cfoutput><cfinclude template="../views/ajaxparts/dsp_pref_global_db.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 22 and right(cfcatch.MissingFileName,22) is "dsp_pref_global_db.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_pref_global_db.cfm in circuit ajax which does not exist (from fuseaction ajax.prefs_global_db).">
