...
<!--- circuit: c --->
<!--- fuseaction: mainchecklist --->
<cftry>
<cfset myFusebox.thisPhase = "appinit">
<cfset myFusebox.thisCircuit = "c">
<cfset myFusebox.thisFuseaction = "mainchecklist">
...
<cfset chklist_settings = myFusebox.getApplicationData().Settings.allsettings_2() >
<cfset tools = myFusebox.getApplicationData().Settings.get_tools() >
<!--- do action="ajax.mainchecklist" --->
<cfset myFusebox.thisCircuit = "ajax">
<cftry>
<cfoutput><cfinclude template="../views/ajaxparts/dsp_main_checkinstall.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 25 and right(cfcatch.MissingFileName,25) is "dsp_main_checkinstall.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_main_checkinstall.cfm in circuit ajax which does not exist (from fuseaction ajax.mainchecklist).">
