					...
					<cfif kind EQ "img">
						<div class="assetbox" style="<cfif cs.assetbox_width NEQ "">width:#cs.assetbox_width#px;</cfif><cfif cs.assetbox_height NEQ "">min-height:#cs.assetbox_height#px;</cfif>" id="#theid#-#kind##iif(isalias EQ 1,de('_alias'),de(''))#">
							<cfif is_available>
								...
								<cfelse><cfset var inventory="#filename#">
								</cfif>
								<cfif structkeyexists(attributes,"share") AND attributes.share EQ "F">
									<cfloop list="#attributes.cs_place.top.image#" index="m" delimiters=",">
										<cfif m CONTAINS "_filename">
											...
											<a href="##" onclick="showwindow('#myself##xfa.detailimg#&file_id=#theid#&what=images&loaddiv=#attributes.thediv#&folder_id=#folder_id_r#&showsubfolders=#attributes.showsubfolders#','#Jsstringformat("#inventory#")#',1200,1);return false;"><strong>#evaluate(listlast(m," "))#</strong></a>
									...
									<a href="##" onclick="showwindow('#myself##xfa.detailimg#&file_id=#theid#&what=images&loaddiv=#attributes.thediv#&folder_id=#folder_id_r#&showsubfolders=#attributes.showsubfolders#','#Jsstringformat("#inventory#")#',1070,1);return false;">
								</cfif>
								...
								<cfif structkeyexists(attributes,"share") AND attributes.share EQ "F">
									...
									<cfif attributes.cs.images_metadata EQ "" OR ( NOT prefs.set2_upc_enabled AND attributes.cs.images_metadata EQ "img_upc_number AS cs_img_upc_number" )>
										<a href="##" onclick="showwindow('#myself##xfa.detailimg#&file_id=#theid#&what=images&loaddiv=#attributes.thediv#&folder_id=#folder_id_r#&showsubfolders=#attributes.showsubfolders#','#Jsstringformat("#inventory#")#',1070,1);return false;"><cfif inventory NEQ ""><strong>#left(inventory,50)#</strong><cfelse><strong>#left(filename,30)#</strong></cfif></a>
									<cfelse>
										<br />
										<cfloop list="#attributes.cs_place.bottom.image#" index="m" delimiters=",">
											<cfif m CONTAINS "_filename">
												<span class="assetbox_title">#myFusebox.getApplicationData().defaults.trans("#listlast(m," ")#")#</span>
												<a href="##" onclick="showwindow('#myself##xfa.detailimg#&file_id=#theid#&what=images&loaddiv=#attributes.thediv#&folder_id=#folder_id_r#&showsubfolders=#attributes.showsubfolders#','#Jsstringformat("#inventory#")#',1070,1);return false;"><strong>#evaluate(listlast(m," "))#</strong></a>
											<cfelseif m CONTAINS "_size">
												<span class="assetbox_title">#myFusebox.getApplicationData().defaults.trans("#listlast(m," ")#")#</span>
												#myFusebox.getApplicationData().global.converttomb('#evaluate(listlast(m," "))#')# MB
											<cfelseif m CONTAINS "_time">
