...
<cffunction name="getfoldersfortree" access="public" output="true">
	...
	<cfif arguments.thestruct.actionismove EQ "F">
		...
		<li id="<cfif iscol EQ "T">col-</cfif>#folder_id#"<cfif subhere EQ "1"> class="closed"</cfif>><a href="##" onclick="loadcontent('rightside','index.cfm?fa=<cfif iscol EQ "T">c.collections<cfelse>c.folder</cfif>&col=F&folder_id=<cfif iscol EQ "T">col-</cfif>#folder_id#');" rel="prefetch" title="<cfif theid EQ 0><cfif iscol EQ "F"><cfif session.theuserid NEQ folder_owner AND folder_owner NEQ ""><!---Folder of (#username#)--->Dossier appartenant &agrave; #username#</cfif></cfif></cfif>"><ins>&nbsp;</ins>#folder_name#<cfif theid EQ 0><cfif iscol EQ "F"><cfif session.theuserid NEQ folder_owner AND folder_owner NEQ "">*<cfif folder_name EQ "my folder"> (#username#)</cfif></cfif></cfif></cfif>
		</a></li>
		</cfif>
		</cfoutput>
					...
					<cfelseif session.type EQ "movefile">
						<cfif session.thefolderorg NEQ folder_id>
							<cfif arguments.thestruct.kind EQ "search">
								...
								<a href="##" onclick="$('##div_choosefolder_status_#session.tmpid#').load('index.cfm?fa=#session.savehere#&folder_id=#folder_id#&folder_name=#URLEncodedFormat(folder_name)#', function(){$('##div_choosefolder_status_#session.tmpid#').html('Les fichiers sont en cours de d&eacute;placement.<br />Remarque: si beaucoup de fichiers ont &eacute;t&eacute; d&eacute;plac&eacute;s, un certain temps peut &ecirc;tre n&eacute;cessaire pour que cela se refl&egrave;te dans le syst&egrave;me !<br />Vous pouvez fermer cette fen&ecirc;tre.');});" style="white-space:normal;">
							<cfelse>
								...
								<a href="##" onclick="$('##div_forall').load('index.cfm?fa=#session.savehere#&folder_id=#folder_id#', function(){$('##div_choosefolder_status_#session.tmpid#').html('Les fichiers sont en cours de d&eacute;placement.<br />Remarque: si beaucoup de fichiers ont &eacute;t&eacute; d&eacute;plac&eacute;s, un certain temps peut &ecirc;tre n&eacute;cessaire pour que cela se refl&egrave;te dans le syst&egrave;me !<br />Vous pouvez fermer cette fen&ecirc;tre.');});" style="white-space:normal;">
							</cfif>
						</cfif>
					<!--- movefolder --->
					...
					<cfelseif session.type EQ "alias">
						<cfif session.thefolderorg NEQ folder_id>
							<cfif arguments.thestruct.kind EQ "search">
								...
								<a href="##" onclick="$('##div_choosefolder_status_#session.tmpid#').load('index.cfm?fa=#session.savehere#&folder_id=#folder_id#&folder_name=#URLEncodedFormat(folder_name)#', function(){$('##div_choosefolder_status_#session.tmpid#').html('L\'alias a &eacute;t&eacute; cr&eacute;&eacute; dans le dossier s&eacute;lectionn&eacute;.<br />Note:Si vous voulez cr&eacute;er un alias pour le même fichier dans un autre dossier, s&eacute;lectionner-le dans dans la liste ci-dessus !<br /> Vous pouvez fermer cette fen&ecirc;tre.');});">
							<cfelse>
								...
								<a href="##" onclick="$('##div_forall').load('index.cfm?fa=#session.savehere#&folder_id=#folder_id#', function(){$('##div_choosefolder_status_#session.tmpid#').html('L\'alias a &eacute;t&eacute; cr&eacute;&eacute; dans le dossier s&eacute;lectionn&eacute;.<br />Note:Si vous voulez cr&eacute;er un alias pour le même fichier dans un autre dossier, s&eacute;lectionner-le dans dans la liste ci-dessus !<br /> Vous pouvez fermer cette fen&ecirc;tre.');});">
							</cfif>
						</cfif>
					</cfif>
